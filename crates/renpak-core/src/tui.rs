...
impl ProgressReport for ChannelProgress {
    fn phase_start(&self, total: u32, msg: &str) {
        let _ = self.tx.send(BuildMsg::PhaseStart {
            total,
            msg: msg.to_string(),
        });
    }
    ...
    fn phase_end(&self, _total: u32, msg: &str, _orig: u64, _comp: u64) {
        let _ = self.tx.send(BuildMsg::PhaseEnd {
            msg: msg.to_string(),
        });
    }
...
fn classify_dirs(rpa_path: &Path) -> Result<(Vec<DirInfo>, u32, u64, u32, u32, bool), String> {
    ...
    let default_skip: Vec<String> = DEFAULT_SKIP_PREFIXES
        .iter()
        .map(|s| s.to_string())
        .collect();
    ...
    fn flatten(
        ...
        children.sort_by(|a, b| {
            ...
            ae.cmp(&be)
                .reverse()
                .then(b.1.subtree_bytes().cmp(&a.1.subtree_bytes()))
        });
        ...
        for (seg, child) in children {
            ...
            let excluded = skip
                .iter()
                .any(|p| full.to_ascii_lowercase().starts_with(p));
...
impl App {
    fn new(game_dir: &Path) -> Result<Self, String> {
        ...
        let mut app = App {
            ...
            cancelling: false,
            has_cache,
            click: RefCell::new(ClickRegions::default()),
            focus: 0,
            action_idx: 0,
            wants_quit: false,
            already_compressed,
        ...
        if already_compressed {
            let backup_exists = game_sub_dir
                .join(".renpak_backup")
                .join(app.rpa_path.file_name().unwrap())
                .exists();
    ...
    fn start_build(&mut self) {
        ...
        if let Ok(avail) = fs2::available_space(game_dir) {
    ...
    fn worker_tier_label(&self) -> &'static str {
        let tiers = self.worker_tiers();
        if self.workers <= tiers[0] {
            "Low"
        } else if self.workers <= tiers[1] {
            "Medium"
        } else {
            "High"
        }
        ...
        match &self.phase {
            Phase::Analyze => match self.focus {
                0 => match code {
                    ...
                    KeyCode::Char(' ') if self.selected < self.dirs.len() => {
                        let new_state = !self.dirs[self.selected].excluded;
                        let parent = self.dirs[self.selected].prefix.clone();
                        for d in &mut self.dirs {
                            if d.prefix.starts_with(&parent) {
                                d.excluded = new_state;
                            }
                        }
                    }
                    KeyCode::Enter
                        if self.selected < self.dirs.len()
                            && self.dirs[self.selected].has_children =>
                    {
                ...
                3 => match code {
                    // Actions block
                    KeyCode::Left | KeyCode::Char('h') if self.action_idx > 0 => {
                        self.action_idx -= 1;
                    }
                    KeyCode::Right | KeyCode::Char('l') => {
                        let max = self.action_count().saturating_sub(1);
                        if self.action_idx < max {
                            self.action_idx += 1;
                        }
                    }
                    KeyCode::Enter | KeyCode::Char(' ') if self.action_idx == 0 => {
                        self.start_build();
                    }
                    _ => {}
                },
    ...
    fn handle_mouse(&mut self, event: MouseEvent) {
            ...
            MouseEventKind::ScrollUp if matches!(self.phase, Phase::Analyze) => {
                ...
                if step > 0 {
                    ...
                    if let Some(pos) = self.vis_pos() {
                        if pos >= vis_end {
                            self.selected =
                                self.visible[vis_end.saturating_sub(1).min(self.visible.len() - 1)];
                        }
                    }
                }
                return;
                ...
                if step > 0 {
                    ...
                    if let Some(pos) = self.vis_pos() {
                        if pos < self.scroll_offset {
                            self.selected =
                                self.visible[self.scroll_offset.min(self.visible.len() - 1)];
                        }
                    }
                }
                return;
        ...
        match &self.phase {
            Phase::Analyze => {
                if hit(cr.preset_high, col, row) {
                    drop(cr);
                    self.preset = QualityPreset::High;
                    return;
                }
                if hit(cr.preset_medium, col, row) {
                    drop(cr);
                    self.preset = QualityPreset::Medium;
                    return;
                }
                if hit(cr.preset_low, col, row) {
                    drop(cr);
                    self.preset = QualityPreset::Low;
                    return;
                }
                if hit(cr.perf_low, col, row) {
                    drop(cr);
                    self.workers = self.worker_tiers()[0];
                    return;
                }
                if hit(cr.perf_medium, col, row) {
                    drop(cr);
                    self.workers = self.worker_tiers()[1];
                    return;
                }
                if hit(cr.perf_high, col, row) {
                    drop(cr);
                    self.workers = self.worker_tiers()[2];
                    return;
                }
                if hit(cr.start_btn, col, row) {
                    drop(cr);
                    self.start_build();
                    return;
                }
                if hit(cr.quit_btn, col, row) {
                    drop(cr);
                ...
                if let Some(area) = cr.dir_list_area {
                        ...
                        && col < area.x + area.width - 1
                    {
                        ...
                        if vis_idx < self.visible.len() {
                            ...
                            } else if self.dirs[dir_idx].has_children
                                && col >= expand_start
                                && col < expand_end
                            {
            ...
            Phase::Done(result) => {
                ...
                if cancelled {
                    ...
                    if hit(cr.install_btn, col, row) {
                        drop(cr);
                        self.handle_action('i');
                        return;
                    }
                } else {
                    if hit(cr.launch_btn, col, row) {
                        drop(cr);
                        self.handle_action('l');
                        return;
                    }
                    if hit(cr.revert_btn, col, row) {
                        drop(cr);
                        self.handle_action('r');
                        return;
                    }
                    if hit(cr.delete_btn, col, row) {
                        drop(cr);
    ...
    fn draw_analyze(&mut self, frame: &mut Frame) {
        ...
        if let Some(msg) = &self.status_msg {
            ...
            frame.render_widget(
                Paragraph::new(Line::from(vec![
                    " Exclude UI, icons & small assets -- poor AVIF quality, minimal savings"
                        .into(),
                ]))
        ...
        let items: Vec<ListItem> = self
            ...
            .map(|(_vi, &di)| {
                ...
                let expand_style = if dim {
                    Style::default().fg(Color::DarkGray)
                } else {
                    Style::default().fg(Color::Yellow)
                };
                let stat_style =
                    Style::default().fg(if dim { Color::DarkGray } else { Color::Gray });
            ...
            .collect();
        let dir_border = if self.focus == 0 {
            Color::Cyan
        } else {
            Color::DarkGray
        };
        ...
        let quality_border = if self.focus == 1 {
            Color::Cyan
        } else {
            Color::DarkGray
        };
        ...
        frame.render_widget(
            preset_btn(QualityPreset::Medium, self.preset),
            preset_cols[3],
        );
        frame.render_widget(preset_btn(QualityPreset::Low, self.preset), preset_cols[5]);
        frame.render_widget(
            Paragraph::new(self.preset.desc().to_string())
        ...
        let perf_border = if self.focus == 2 {
            Color::Cyan
        } else {
            Color::DarkGray
        };
        ...
        frame.render_widget(
            perf_btn("Low", self.worker_tier_label() == "Low"),
            perf_cols[1],
        );
        frame.render_widget(
            perf_btn("Medium", self.worker_tier_label() == "Medium"),
            perf_cols[3],
        );
        frame.render_widget(
            perf_btn("High", self.worker_tier_label() == "High"),
            perf_cols[5],
        );
        ...
        let action_border = if self.focus == 3 {
            Color::Cyan
        } else {
            Color::DarkGray
        };
        ...
        let skip_count = self
            .dirs
            .iter()
            .filter(|d| d.excluded && d.own_count > 0)
            .count();
        frame.render_widget(
            Paragraph::new(Line::from(vec![
    ...
    fn draw_building(&self, frame: &mut Frame) {
        ...
        if !p.warnings.is_empty() {
            let warns: Vec<ListItem> = p
                .warnings
                .iter()
                .rev()
                .take(5)
                .map(|w| {
                    ListItem::new(Span::styled(w.as_str(), Style::default().fg(Color::Yellow)))
                })
                .collect();
            let warn_list = List::new(warns).block(
                Block::bordered()
                    .title(" Warnings ")
        ...
        match result {
            Ok(stats) if stats.cancelled => {
                ...
                let body = Paragraph::new(vec![
                    Line::from(""),
                    Line::from(format!(
                        "  Encoded {}/{} images before cancel",
                        stats.encoded, n_images
                    )),
                    Line::from(
                        format!("  {} cached -- resume will skip these", stats.encoded).dark_gray(),
                    ),
                    Line::from(""),
                ...
                frame.render_widget(
                    btn(" Resume ", Color::Yellow, self.action_idx == 0),
                    btn_cols[1],
                );
                frame.render_widget(
                    btn(" Quit ", Color::Gray, self.action_idx == 1),
                    btn_cols[3],
                );
                let mut cr = self.click.borrow_mut();
                cr.start_btn = Some(btn_cols[1]);
            ...
            Ok(stats) => {
                ...
                let body = if self.already_compressed {
                    ...
                    Paragraph::new(vec![
                        ...
                        Line::from(vec![
                            ...
                            "  pass ".dark_gray(),
                            format!("{:.1}s", t.passthrough_s).into(),
                            "  cache ".dark_gray(),
                            format!("{:.1}s", t.cache_s).into(),
                            "  encode ".dark_gray(),
                            format!("{:.1}s", t.encode_s).into(),
                            "  finalize ".dark_gray(),
                            format!("{:.1}s", t.finalize_s).into(),
                        ]),
                        Line::from(vec![format!("  Total: {}", fmt_duration(t.total_s)).into()]),
                    ])
                    .block(Block::bordered().border_style(Style::default().fg(Color::DarkGray)))
                };
                frame.render_widget(body, layout[1]);
                ...
                if let Some(msg) = &self.status_msg {
                    let style = if msg.contains("ailed") {
                        Style::default().fg(Color::Red)
                    } else {
                        Style::default().fg(Color::Yellow)
                    };
                ...
                let btn_rows = Layout::vertical([Constraint::Length(1), Constraint::Length(1)])
                    .split(action_inner);
                ...
                if self.installed {
                    ...
                    .split(btn_rows[1]);
                    frame
                        .render_widget(btn(" Launch ", Color::Cyan, self.action_idx == 0), cols[1]);
                    frame.render_widget(
                        btn(" Revert ", Color::Yellow, self.action_idx == 1),
                        cols[3],
                    );
                    ...
                    .split(btn_rows[1]);
                    frame.render_widget(
                        btn(" Install ", Color::Green, self.action_idx == 0),
                        cols[1],
                    );
            ...
            Err(msg) => {
                ...
                let body =
                    Paragraph::new(vec![Line::from(""), Line::from(format!("  {msg}").red())])
                        .block(
